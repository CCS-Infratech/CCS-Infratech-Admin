  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        // Populate overview
        setOverviewHeadline(project.overviewHeadline || '');
        setOverviewDescription(project.overviewDescription || '');
        ...
        // Populate site plan
        setSitePlanHeadline(project.sitePlanHeadline || '');
        if (project.sitePlanImage) {
          setSitePlanImage({
            url: project.sitePlanImage,
            key: project.sitePlanImage.split('/').pop() || '',
            size: 0,
            lastModified: '',
            type: 'image'
          });
        }
        if (project.sitePlanBrochureUrl) {
          setSitePlanBrochureUrl({
            url: project.sitePlanBrochureUrl,
            key: project.sitePlanBrochureUrl.split('/').pop() || '',
            size: 0,
            lastModified: '',
            type: 'pdf'
          });
        ...
        setCurrentPlanHeadline(project.currentPlanHeadline || '');
        if (project.currentPlanImage) {
          setCurrentPlanImage({
            url: project.currentPlanImage,
            key: project.currentPlanImage.split('/').pop() || '',
            size: 0,
            lastModified: '',
            type: 'image'
          });
        }
        if (project.currentPlanBrochureUrl) {
          setCurrentPlanBrochureUrl({
            url: project.currentPlanBrochureUrl,
            key: project.currentPlanBrochureUrl.split('/').pop() || '',
            size: 0,
            lastModified: '',
            type: 'pdf'
          });
        ...
        // Populate unit plan
        setUnitPlanHeadline(project.unitPlanHeadline || '');
        if (project.unitPlanImage) {
          setUnitPlanImage({
            url: project.unitPlanImage,
            key: project.unitPlanImage.split('/').pop() || '',
            size: 0,
            lastModified: '',
            type: 'image'
          });
        }
        if (project.unitPlanBrochureUrl) {
          setUnitPlanBrochureUrl({
            url: project.unitPlanBrochureUrl,
            key: project.unitPlanBrochureUrl.split('/').pop() || '',
            size: 0,
            lastModified: '',
            type: 'pdf'
          });
        ...
        setNearbyAttractions(project.nearbyAttractions || '');
        setLocationDetails(project.locationDetails || '');
        ...
        if (project.images && project.images.length > 0) {
          setGalleryImages(
            project.images.map((img: any) => ({
              image: {
                url: img.url,
                key: img.filename || img.url.split('/').pop() || '',
                size: 0,
                lastModified: '',
                type: 'image'
              },
  ...
  if (loading) {
    return (
      <div className='flex h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-white'>
        <div className='text-center'>
          <Loader2 className='mx-auto h-12 w-12 animate-spin text-[#b07d17]' />
            ...
            <div className='flex items-center gap-3'>
              ...
              <div>
                ...
                <p className='mt-0.5 hidden text-xs text-gray-500 sm:block sm:text-sm'>
                  Update and republish your project
                </p>
              </div>
            </div>
            <Button
              ...
              <div className='space-y-4 sm:space-y-5'>
                <div className='mb-4 flex items-center justify-between'>
                  ...
                  <Button
                    ...
                    size='sm'
                    className='rounded-lg bg-gradient-to-r from-[#b07d17] to-[#c89420] shadow-lg shadow-[#b07d17]/30 sm:rounded-xl'
                  >
                    ...
                    <div className='space-y-4'>
                      <div>
                        ...
                        <Input
                          ...
                          className='rounded-lg border-gray-200 text-sm focus:border-[#b07d17] focus:ring-[#b07d17] sm:rounded-xl'
                        />
                      </div>
